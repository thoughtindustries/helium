...
const Hero = (props: HeroProps): JSX.Element => {
  const { title, subtitle, linkText, linkOpenInNewTab, linkUrl, asset, largeAsset, smallAsset } =
    props;
  const image = title && (
  ...
  if (title) {
    ...
    caption = (
      <div className="w-full p-8 md:absolute md:left-0 md:bg-white md:bg-opacity-80 md:bottom-8">
        ...
        {subtitle && (
          <span className="text-gray-800 text-xs leading-5 font-header md:text-base lg:text-lg">
            {subtitle}
          </span>
        )}
        {link}
