      ...
      {loading ? (
        ...
        data.UserContentItems.map((item: any) => {
          return (
            <div
              key={item.id}
              className="border-solid p-4 text-black-light border-gray-light px-4 py-[0.5rem]"
            >
              <div className="my-0 mx-auto max-w-full w-full">
                <div className="grid items-center grid-cols-12 gap-4">
                  ...
                  <div className="col-span-2">{new Date(item.displayDate)}</div>

                  <div className="col-span-3 text-gray-mid">
                    <strong>{item.contentTypeLabel}</strong>
                  </div>
